...
void FDMatrices::BuildLaplacianMatrix(Grid &g, SpMat &L)
{
    ...
    for(int row = 0; row < g.sizeBand(); ++row)
    {
        ...
        {
            if(g.bandNode(row).neighbour(nbr) != nullptr)
            {
                coeffs.push_back(T(row, g.bandNode(row).neighbour(nbr)->bandIndex(), 1.0)); // neighour entries
            }
        }
    }
...
void FDMatrices::BuildGradientMatrices(Grid &g, vector<SpMat> &Dc)
{
    ...
    for(int row = 0; row < g.sizeBand(); ++row)
    {
        for(int nbr = 0; nbr < 2 * g.dim(); ++nbr)
        {
            if(g.bandNode(row).neighbour(nbr) != nullptr)
            {
                coeffs[nbr/2].push_back(T(row, g.bandNode(row).neighbour(nbr)->bandIndex(), pow(-1, nbr) * 0.5)); // neighour entry
            }
        }
    }
